...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            is_relevant = (overlap / len(expected_tokens)) >= relevance_threshold
            relevant_indicators.append(is_relevant)
            
        num_relevant = sum(1 for r in relevant_indicators if r)
        if num_relevant == 0:
            return 0.0
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = (
            f"Please evaluate the answer based on the question and the rubric provided below.\n\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n"
        )
        ...
        prompt += (
            f"\nYour response must be in JSON format where keys are the criteria name and values are floats between 0.0 and 1.0.\n"
            f"Example: {{\"criterion1\": 0.8, \"criterion2\": 0.9}}"
        )
        ...
        import json
        try:
            # First look for JSON block or any JSON-like dict in the response
            match = re.search(r"\{.*\}", response, re.DOTALL)
            if match:
                data = json.loads(match.group(0))
                # Check if it has 'scores' key or is scores directly
        ...
        # Ensure all criteria in rubric are present in the scores.
        # If any criterion is missing, default to 0.5.
        final_scores = {}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            sc = entry.get("scores", {})
            if sc:
                item_averages.append(sum(sc.values()) / len(sc))
        ...
        avg_all = sum(item_averages) / len(item_averages)
        leniency_bias = avg_all > 0.8
        severity_bias = avg_all < 0.3
        
        # Positional bias check: if even-indexed items (first position in pairs)
        # consistently score higher than odd-indexed items (second position)
        if len(item_averages) >= 2:
            even_avg = sum(item_averages[::2]) / len(item_averages[::2])
            odd_avg = sum(item_averages[1::2]) / len(item_averages[1::2])
            positional_bias = even_avg > odd_avg
        else:
            positional_bias = False
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Re-associate with the original pair so metadata and retrieved_contexts are preserved.
            res.qa_pair = pair
            # Retrieve-side metrics if chunks are present:
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # If there's a tie for the lowest score, return multiple issues detected:
    ...
    def generate_improvement_suggestions(
        ...
        # If irrelevant is present
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Add system prompt instructions to focus strictly on the query context")
            
        # If off_topic is present
        if categories.get("off_topic", 0) > 0:
            suggestions.append("Implement intent detection to route off-topic queries to a default response handler")
            
        # Default suggestions to make sure we always have at least 3 if failures is not empty
        default_suggestions = [
            "Implement hallucination checker to filter unsupported claims",
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Tune prompt to penalize long-winded irrelevant explanations",
        ]
